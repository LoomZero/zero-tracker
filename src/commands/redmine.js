...
module.exports = class RedmineCommand extends Command {
  ...
  async add() {
    ...
    this.tracker.config.set('redmine.' + project + '.hostname', await Input.input('[hostname] Please enter the Redmine Hostname (Select "https://projects.loom.de/" for LOOM redmine): ', Input.optionsNotEmpty()));
    ...
    this.tracker.config.set('redmine.' + project + '.alias', await Input.input('[alias] Optional enter an alias ("," seperated): ', Input.optionsTermArray({}, ',', {trimItems: true})));
    this.tracker.config.save();
  }
